...
const Comment: React.FC<{ event: Event }> = ({ event: { content, tags, pubkey }}) => {
...
const CommentsScreen: React.FC<{
  ...
  const descriptionTags = comments.map(({ tags }) => {
  ...
  const parseZapRequestJSON = async (json: string[]) => await Promise.all(json.map(async content => JSON.parse(content)));

  useEffect(() => {
    parseZapRequestJSON(descriptionTags as string[])
    .then(events => setParsedZapRequests(events))
  }, [comments]);
